...
# Function to filter items based on calorie range and price range
# filtered_items = filter_items(data, category, goal, price, distance)

def filter_items(type, calorie_type, price_type, distance):
...
def findItemfromMenu(restaurant, category): 
    print("Grabbing menu for ", restaurant)
    #this will read the menu for input.json and if it finds the restaurant in the "Restaurant" tag it will print the item 
    with open('input.json', 'r') as f:
        data = json.load(f)
        for item in data:
            if (item['Restaurant']== restaurant and item['Category'] == category):
                print(item['Item'])
                print(item['Category'])
                print(item['Price'])
                print(item['Calories'])
...
def filter_items(data, type, calorie_type, price_type, distance):
    # Define calorie ranges
    if calorie_type == 'low':
        calorie_range = (0, 300)
    elif calorie_type == 'medium':
